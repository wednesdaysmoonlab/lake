...
- **Agent MCP command paths (`_patch_agent_mcp_php`)** — MCP installers (`laravel/boost`'s `boost:install`, etc.) write each agent's config with a bare `command: php`, which has no host runtime to resolve. `_patch_agent_mcp_php` rewrites every known agent config to the **absolute** `.lake/php` shim — the exact value Boost itself writes once `BOOST_PHP_EXECUTABLE_PATH` is set, so it repairs configs left over from a `boost:install` that ran *before* the `.env` seed (`_seed_boost_php_path`). Covers all ten files Boost can write: `.mcp.json`, `opencode.json`, `.codex/config.toml`, `.amp/settings.json`, `.zed/settings.json`, `.factory/mcp.json`, `.cursor/mcp.json`, `.vscode/mcp.json`, `.junie/mcp/mcp.json`, `.kiro/settings/mcp.json` — across all three formats (JSON `"command": "php"` string, OpenCode's `"command": [ "php", … ]` array, Codex's TOML `command = "php"`). **Pure bash + `sed`** (no `frankenphp` needed), idempotent (only a bare `php` command changes; a non-command `"php"` string is left alone), runs unprompted on every bootstrap. The helper is mirrored in `tests/helpers/patch_agent_mcp` (kept in sync) and covered by `tests/unit/agent_mcp.bats`. *Historical note:* this previously patched only `.mcp.json` via `frankenphp php-cli -r`, which left Codex/OpenCode/Zed/Amp/… pointing at a non-existent `php`.
